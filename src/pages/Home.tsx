    ...
    <>
      ...
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {/* Real client logo strip */}
          <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6 mb-14 opacity-80">
            {[
              ...
              <img
                ...
                alt={logo.name}
                loading="lazy"
                className="h-8 sm:h-9 w-auto max-w-[110px] object-contain grayscale hover:grayscale-0 transition-all duration-300"
              />
            ))}
          </div>
      ...
      <motion.section
        id="stat-strip"
        className="py-16 bg-neutral-900 font-sans"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: '-60px' }}
        transition={{ duration: 0.6 }}
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="text-center text-xs font-bold uppercase tracking-widest text-neutral-500 mb-10">
            The 8 case studies above are a sample, not the whole picture
          </p>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {[
              { stat: '75+', label: 'Businesses Helped' },
              { stat: '8', label: 'Featured Case Studies' },
              { stat: '100%', label: 'Client-Owned Code' },
              { stat: '1-on-1', label: 'Founder-Direct, No Account Managers' },
            ].map((item, idx) => (
              <motion.div
                key={item.label}
                initial={{ opacity: 0, y: 16 }}
